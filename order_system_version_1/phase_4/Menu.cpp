...
namespace seneca
{
    ...
    {
        // REVISED: Set proper indentation values to match example output
        // For Food Menu with indentation 2, we need 6 spaces (2*3)
        indentNumber = indent_Number;
        indentSize = indent_Size;
    ...
    void Menu::displaySubTitle() const
    {
        if (title.menuItemContent != nullptr)
        {
            // Apply consistent indentation for subtitles
            for (unsigned int i = 0; i < indentNumber * indentSize; i++)
            {
                cout << " ";
            }
